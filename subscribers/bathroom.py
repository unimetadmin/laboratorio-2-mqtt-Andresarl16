...
def on_connect(client, userdata, flags, rc):
	...
	client.subscribe(topic='house/bathroom/#', qos=2)

def on_message(client, userdata, message):
	if("tank" in message.topic):
		message = json.loads(message.payload)

		if("waterLevel" in message):
			query = 'INSERT INTO public.water_tank(date , water_level) VALUES (%s, %s);'
			data = (message["date"],int(round(float(message["waterLevel"]), 0)))
			select(query, data)
			print(message)
...
def main():
	client = paho.mqtt.client.Client(client_id='bathroom', clean_session=False)
